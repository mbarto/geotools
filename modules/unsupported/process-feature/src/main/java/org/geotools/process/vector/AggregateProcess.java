...
public class AggregateProcess implements VectorProcess {
    ...
    private Results handleGroupByVisitor(SimpleFeatureCollection features, String aggAttribute, Set<AggregationFunction> functions,
        ...
        List<GroupByVisitor> groupByVisitors = functions.stream().map(function -> function == AggregationFunction.SumArea ? new GroupByVisitorBuilder()
                .withAggregateAttribute(factory.function("area2", factory.property(features.getSchema().getDescriptor(aggAttribute).getLocalName())))
                .withAggregateVisitor(function.name())
                .withGroupByAttributes(rawGroupByAttributes, features.getSchema())
                .withProgressListener(progressListener)
                .build() : new GroupByVisitorBuilder()
